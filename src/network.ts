...
export interface NetworkState {
  ...
  deployments: Partial<Record<NetworkId, Record<DeploymentKey, DeploymentRecord>>>;
}
...
export function getDeployment(network: NetworkId, contractName: string = 'whistleblower', opts: FsOptions = {}): DeploymentRecord | null {
  ...
  return (networkDeployments as Record<string, DeploymentRecord>)[contractName] ?? null;
}
...
export function recordContractDeployment(
  ...
  const cwd = opts.cwd ?? process.cwd();
  const existing = loadState({ cwd });
  const oldDeployments = existing?.deployments?.[network];
  let migratedDeployments: Record<string, DeploymentRecord> = {};
  if (oldDeployments && typeof oldDeployments === 'object' && !Array.isArray(oldDeployments)) {
    for (const [k, v] of Object.entries(oldDeployments as Record<string, unknown>)) {
      if (v && typeof v === 'object' && 'address' in v) {
        migratedDeployments[k] = v as DeploymentRecord;
      }
    }
  }
  const next: NetworkState = existing ?? {
  ...
  next.deployments = {
    ...
    [network]: {
      ...migratedDeployments,
      [contractName]: { address, deployer, deployedAt: new Date().toISOString() },
    },
  };
  saveState(next, { cwd });
}
