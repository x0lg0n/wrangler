...
import { ArrowRight, CheckCircle2, Loader2, XCircle, X } from 'lucide-react';
          ...
          <>
            ...
            <div className="flex flex-col gap-2.5">
              {wallets.map((w) => (
                ...
                >
                  {w.icon && (
                    <img src={w.icon} alt="" className="size-9 rounded-md" />
                  )}
                  <div className="flex-1">
                    <div className="text-sm font-semibold text-foreground">{w.name}</div>
                    <div className="mt-0.5 text-[11px] text-muted-foreground">{w.rdns}</div>
                  </div>
            ...
            <button
              onClick={onClose}
              className="mt-2.5 flex w-full items-center justify-center rounded-md border border-border bg-background px-4 py-2.5 text-sm font-medium text-muted-foreground transition-colors hover:border-bg-hover hover:text-secondary-foreground"
            >
              Cancel
            </button>
          </>
        ...
        {step === 'error' && (
          <div className="text-center">
            ...
            <p className="mb-5 text-sm leading-relaxed text-muted-foreground">
              ...
              <br /><br />
              Install{' '}
              <a href="https://chromewebstore.google.com/detail/lace/cnkempipohcbaddhknkiilgfbhdbfhpn"
                 target="_blank" rel="noopener noreferrer"
                 className="text-accent underline">
                Lace Wallet
              </a>
              {' '}or{' '}
              <a href="https://1amwallet.com"
                 target="_blank" rel="noopener noreferrer"
                 className="text-accent underline">
                1AM Wallet
              </a>
              {' '}from your browser extension store, then refresh and try again.
            </p>
            <div className="flex justify-center gap-2.5">
              <Button
                onClick={() => { setStep('detecting'); setError(null); setWallets(detectWallets()); }}
              >
                Retry
              </Button>
